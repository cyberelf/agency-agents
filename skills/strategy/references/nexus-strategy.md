...
## The Agency's Complete Operational Playbook for Multi-Team Orchestration
...
10. [Team Coordination Matrix](#10-team-coordination-matrix)
...
Individual teams are powerful. But without coordination, they produce:
...
- **Why** each team exists in the pipeline (no passengers)
...
| **Context Continuity** | Every handoff carries full context — no team starts cold |
...
### 1.3 The Team Skill Roster by Division

| Division | Teams | Primary NEXUS Role |
|----------|--------|--------------------|
...
| **Specialized** | Strategy Team Orchestrator, Data Analytics Reporter, LSP/Index Engineer, Sales Data Extraction Team, Data Consolidation Team, Report Distribution Team | Cross-cutting coordination, deep analytics, and code intelligence |
...
```
                    ┌──────────────────────┐
                    │  Strategy Team Orchestrator  │  ◄── Pipeline Controller
                    │  (Specialized)        │
                    └──────────┬───────────┘
                               │
              ┌────────────────┼────────────────┐
...
| Mode | Teams Active | Use Case | Timeline |
|------|--------------|----------|----------|
...
| Team | Role in Phase | Primary Output |
|-------|--------------|----------------|
...
This is the heart of NEXUS. The Strategy Team Orchestrator manages a **task-by-task quality loop**:
...
### 6.2 Team Assignment by Task Type

| Task Type | Primary Developer | QA Team | Specialist Support |
|-----------|------------------|----------|-------------------|
...
**Gate Keeper**: Strategy Team Orchestrator
...
### 9.1 Active Teams (Ongoing)

| Team | Cadence | Responsibility |
|-------|---------|---------------|
...
This matrix shows which teams produce outputs consumed by other teams. Read as: **Row team produces → Column team consumes**.
...
| Evidence Collector | Strategy Team Orchestrator | QA Verdict (PASS/FAIL) | Per task |
| Strategy Team Orchestrator | Developer (any) | QA Feedback + Retry Instructions | Per failure |
...
| Reality Checker | Strategy Team Orchestrator | Integration Verdict | Per phase |
...
### Metadata
- **From**: [Team Name] ([Division])
- **To**: [Team Name] ([Division])
...
### Escalation Level: [To Strategy Team Orchestrator / To Studio Producer]
...
- [ ] Reassign to different developer team
...
| 3 → 4 | Feature Gate | Strategy Team Orchestrator | All tasks pass QA, no critical bugs, performance baselines met |
...
IF gate FAILS:
  ...
  ├── Strategy Team Orchestrator routes failures to responsible teams
...
| Risk Category | Primary Owner | Mitigation Team | Escalation Path |
|---------------|--------------|-------------------|-----------------|
...
| Quality Regression | Reality Checker | Evidence Collector | Strategy Team Orchestrator |
...
| **High** (P1) | < 4 hours | Project Shepherd | Dedicated team assignment |
| **Medium** (P2) | < 24 hours | Strategy Team Orchestrator | Next sprint priority |
...
| Metric | Target | Measurement Team |
|--------|--------|-------------------|
| Phase completion rate | 95% on first attempt | Strategy Team Orchestrator |
| Task first-pass QA rate | 70%+ | Evidence Collector |
| Average retries per task | < 1.5 | Strategy Team Orchestrator |
...
"Activate Strategy Team Orchestrator in NEXUS-Full mode for [PROJECT NAME].
...
# 2. Activate Phase 0 teams for discovery
...
"Activate Strategy Team Orchestrator in NEXUS-Sprint mode for [FEATURE/MVP NAME].
...
# Specialized: Strategy Team Orchestrator
```

### 15.3 NEXUS-Micro Activation (Targeted Task)

```bash
# Step 1: Direct team activation
...
### 15.4 Team Activation Prompt Templates

#### For the Orchestrator (Pipeline Start)
```
You are the Strategy Team Orchestrator running NEXUS pipeline for [PROJECT].
...
1. Read the project specification
2. Activate Phase [N] teams per the NEXUS strategy
...
#### For Developer Teams (Task Implementation)
```
...
#### For QA Teams (Task Validation)
```
You are [QA AGENT] validating work within the NEXUS pipeline.

Phase: [Current Phase]
Task: [Task ID and description]
Developer: [Which team implemented this]
...
| Team | Superpower | Activation Trigger |
|-------|-----------|-------------------|
...
| Team | Superpower | Activation Trigger |
|-------|-----------|-------------------|
| Strategy Team Orchestrator | Multi-team pipeline management | Any multi-team workflow |
...
| Sales Data Extraction Team | Excel monitoring, sales metric extraction | Sales data ingestion |
| Data Consolidation Team | Sales data aggregation, dashboard reports | Territory and rep reporting |
| Report Distribution Team | Automated report delivery | Scheduled report distribution |
...
|------|----------|--------|-------|
| [Description] | [P0-P3] | [Active/Mitigated/Closed] | [Team] |
...
**Orchestrator**: Strategy Team Orchestrator
...
| **Handoff** | Structured transfer of work and context between teams |
| **Gate Keeper** | Team(s) with authority to approve or reject phase advancement |
| **Escalation** | Routing a blocked task to higher authority after retry exhaustion |
| **NEXUS-Full** | Complete pipeline activation with all teams |
| **NEXUS-Sprint** | Focused pipeline with 15-25 teams for feature/MVP work |
| **NEXUS-Micro** | Targeted activation of 5-10 teams for specific tasks |
...
*From discovery to sustained operations — every team knows their role, their timing, and their handoff.*
