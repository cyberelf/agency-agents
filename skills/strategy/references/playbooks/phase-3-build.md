...
> **Duration**: 2-12 weeks (varies by scope) | **Teams**: 15-30+ | **Gate Keeper**: Strategy Team Orchestrator
...
The Strategy Team Orchestrator manages every task through this cycle:

```
FOR EACH task IN sprint_backlog (ordered by RICE score):

  1. ASSIGN task to appropriate Developer Team (see assignment matrix)
  ...
  4. IF verdict == PASS:
     ...
     ELIF verdict == FAIL AND attempts < 3:
       ...
       Return to step 3
     ELIF attempts >= 3:
       ESCALATE to Strategy Team Orchestrator
...
| Task Category | Primary Team | Backup Team | QA Team |
|--------------|--------------|-------------|----------|
...
```
Managed by: Strategy Team Orchestrator (Dev↔QA loop)
Teams: Frontend Developer, Backend Architect, AI Engineer,
...
Teams: Growth Hacker, Content Creator, Social Media Strategist,
        App Store Optimizer
...
```
Managed by: Strategy Team Orchestrator
Teams: Evidence Collector, API Tester, Performance Benchmarker,
...
Teams: UI Designer, Brand Guardian, Visual Storyteller,
        Whimsy Injector
...
3. Assign tasks to developer teams
...
```
Strategy Team Orchestrator manages:
...
Participants: All active teams + stakeholders
...
WHEN task fails QA:
  ...
  IF attempt == 2:
    ...
    → Consider: Is the developer team the right fit?
  ...
  IF attempt == 3:
    → ESCALATE
    → Options:
      a) Reassign to different developer team
...
WHEN multiple tasks have no dependencies:
  → Assign to different developer teams simultaneously
